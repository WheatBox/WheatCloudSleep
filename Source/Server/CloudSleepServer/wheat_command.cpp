...
namespace wheat
{
	...
	WheatCommand ParseCommand(std::string_view msg)
	{
		...
		if (temp1 == NULL || temp2 == NULL
			...
			std::string command = (temp1->valuestring);
			std::string args = (temp2->valuestring);
	...
	std::string PackCommand(const WheatCommand& cmd)
	{
		return std::visit(overloaded{
			...
			[](CmdGetup arg) { return (ProcCommand("getup", "0")) /* + arg.bed_id*/; },
			...
			[](CmdVoteKickOver) { return (ProcCommand("agree", "0")); },
