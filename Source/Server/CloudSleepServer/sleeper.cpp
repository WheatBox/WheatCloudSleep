...
namespace wheat
{
	...
	int Sleeper::RecvIsJSON(std::string msg) {
		return 1;
	}
	...
	asio::awaitable<void> Sleeper::Reader()
	{
		try
		{
			...
			{
				std::string buffer;
				cJSON* pcjson_RecvBuff = cJSON_CreateObject();
				...
				int ret = RecvIsJSON(buffer.data());//
				if (pcjson_RecvBuff == NULL || !ret) {
					LOG_DEBUG("pares fail.BUFFER:%s\n", buffer);
				}
				else
				{
					pcjson_RecvBuff = cJSON_Parse(buffer.data());
					LOG_INFO("RecvBuff:%s\n", cJSON_Print(pcjson_RecvBuff));
				}
